...
public sealed class SdkTests
{
    ...
    public void ExtendedSandboxOptionsUseTheGoCreateContract()
    {
        var json = new SandboxOptions
        {
            ...
            RegistryAuth = new SandboxRegistryAuthOptions
            {
                Username = "user",
                Password = "secret",
            },
            ...
            Network = new SandboxNetworkOptions
            {
                ...
                Tls = new SandboxTlsOptions
                {
                    ...
                    [
                        new SandboxScopedVerifyUpstream("internal.example.com", false),
                    ],
                },
                MaxConnections = 100,
            },
        ...
        False(
            root.GetProperty("network")
                .GetProperty("dns")
                .GetProperty("rebind_protection")
                .GetBoolean()
        );
        False(
            root.GetProperty("network")
                .GetProperty("tls")
                .GetProperty("scoped_verify_upstream")[0]
                .GetProperty("verify")
                .GetBoolean()
        );
        Equal(
        ...
        Equal(
            "data",
            root.GetProperty("volumes").GetProperty("/data").GetProperty("named").GetString()
        );
    }
    ...
    public void StreamingExecOptionsUseTheExactStdinContract()
    {
        using var document = JsonDocument.Parse(
            new ExecOptions { StdinPipe = true, Tty = true }.ToJson()
        );
    ...
    public void SandboxNamesAreLimitedByUtf8Bytes()
    {
        ...
        Throws<ArgumentOutOfRangeException>(() =>
            MicrosandboxClient.ValidateName(new string('é', 65))
        );
    }
    ...
    public void ManagedVersionPreservesPrereleaseIdentifiers(
    ...
    {
        using var clientOptions = JsonDocument.Parse(
        ...
        using var serverOptions = JsonDocument.Parse(
            new SshServerOptions
            {
        ...
        var output = NativeApi.ParseSshOutput(
            """{"status":7,"stdout":"AAH/","stderr":"d2FybmluZw=="}"""
        );
    ...
    public void RawAgentContractsPreserveBytesFlagsAndAtomicOwnership()
    {
        var frame = new RawFrame(
            42,
            AgentFrameFlags.Terminal | AgentFrameFlags.SessionStart,
            [0, 1, 255]
        );
        Equal(42U, frame.Id);
    ...
    public void SandboxModificationUsesTheCanonicalPatchAndPlanContracts()
    {
        ...
        Equal(
            "env",
            patch.GetProperty("secrets")[0].GetProperty("source").GetProperty("kind").GetString()
        );
        ...
        Throws<ArgumentException>(() =>
            new SandboxModificationOptions
            {
                Patch = new SandboxModificationPatch
                {
                    Secrets = new Dictionary<string, SandboxSecretModification>
    ...
    public void NativeAbiSymbolsAndVersionCanBeReadWhenLibraryIsProvided()
    {
        ...
        Throws<MicrosandboxException>(() =>
            native
                .AgentRequestAsync(0, 0, Array.Empty<byte>(), CancellationToken.None)
                .GetAwaiter()
                .GetResult()
        );
    }
