...
public sealed record SandboxOptions
{
    ...
    private static ulong? ToMilliseconds(TimeSpan? value) =>
        value is { } duration ? checked((ulong)Math.Ceiling(duration.TotalMilliseconds)) : null;

    private static ulong? ToSeconds(TimeSpan? value) =>
        value is { } duration ? checked((ulong)Math.Ceiling(duration.TotalSeconds)) : null;
